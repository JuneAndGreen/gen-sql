...
function constructCondition(stmt, table, darr) {
    ...
    if(isString(stmt)) {
        ...
        for(let stmtItem of stmt) {
            ...
            for(let key of keys) {
                // 遍历条件
                let itm = stmtItem[key];

                if (!isNaN(parseInt(key, 10))) {
                    // key 为数字，则以值为条件
                    conditionItem.push(itm);
                } else {
                    let attrName = splitAttrName(key, table);

                    if(isOrigin(itm)) {
                        // 值类型
                        darr.push(itm);
                        conditionItem.push(`${attrName} = ?`);
                    } else if(isArray(itm)) {
                        // 数组类型
                        let fragment = [];
                        for(let val of itm) {
                        ...
                        conditionItem.push(`${attrName} in (${fragment})`);
                        continue;
                    } else {
                        // 其他
                        continue;
 ...
 * 拆分数据库字段值
 */
function splitAttrName(key, table) {
    ...
    find(obj) {
        ...
        if(isObject(field)) {
            ...
            if(isArray(attrs)) {
                ...
                attrs = attrs.map(function(attr) {
                    return splitAttrName(attr, table);
                });
            ...
            if(isObject(map)) {
                ...
                for(let key of keys) {
                    name.push(`${splitAttrName(key, table)} AS \`${map[key]}\``);
                }
            }
        ...
        if(isArray(like)) {
            ...
            for(let itm of like) {
                ...
                for(let attr of attrs) {
                    ...
                    likeCondition.push(`${splitAttrName(attr, table)} LIKE "${fragment}"`);
                }
        ...
        if(isArray(order)) {
            ...
            for(let itm of order) {
                ...
                orderArr.push(`${splitAttrName(attr, table)} ${type}`);
            }
